        ...
        try:
            ...
            # ========================================================
                ...
                if lstm_info is None or lstm_info[0] != "trained":
                    ...
                    st.info(f"RF Pre-training Accuracy: {acc:.3f}")
...
# ---------------- TAB 3 ----------------
with tab3:
    st.header("📍 Address Translation & TLB Demo")
    c1, c2, c3 = st.columns(3)
...
# ---------------- TAB 5 ----------------
with tab5:
    st.header("🔥 Advanced Thrashing Monitor")

    # ------------------------------
    # Controls
    # ------------------------------
    frames_th = st.slider("Frames", 1, 50, 6)
    length_th = st.slider("Reference Length", 50, 3000, 800)
    max_page_th = st.slider("Max Page ID", 10, 200, 50)
    window = st.slider("Window Size", 10, 300, 50)
